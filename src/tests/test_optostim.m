% @title      Test script for optogenetics stimulation
% @file       test_optostim.m
...
%
% @brief Test script for optogenetics stimulation
% 
% @details
...
% Optogenetic external stimulation
    opto_stim = struct(...
        'en'        , true, ...            % Enable optogenetic stimulation
        'ratio'     , 1.0, ...              % Ratio of neurons in bundle affected
        ...
        'pattern'   , [ [0,     -1]; ...    % [ms, Hz] - Associate time to stimulation frequency
                        [300,   1]; ...   % [ms, Hz] - Associate time to stimulation frequency     
                        [1500,   1.5]; ...     % [ms, Hz] - Associate time to stimulation frequency     
                        [2000,   2]; ...   % [ms, Hz] - Associate time to stimulation frequency     
                        [3500,  -1]], ...     % [ms, Hz] - Associate time to stimulation frequency
...
%% Optogenetic stimulation figure
figure('Name','Membrane voltage','NumberTitle','off');
    subplot(311)
        title('Membrane voltage response to optogenetics stimulation');
        for i = 1:MAX_N
            % subplot(MAX_N/2,2,i);
            hold on
            plot(t, V(i,:));
        ...
        yyaxis right
        plot(t, opto_stim_pulse, 'green');
        ylabel('Stim state (ON/OFF)');
        xlabel('Time (ms)');
        ll{i+1} = 'Optostim state';
        legend(ll);
